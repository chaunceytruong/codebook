...
public class ArithmeticExpressionTree<T extends Comparable> extends AbstractBinaryTree<T> {
    ...
    // to a in-order traversal, in which the processing is doing
    // the actual arithmetic.  For isLeaf nodes, it is simply the
    // value of the node.
    private double evaluate(BinaryNode<T> node) {
        double result;	// Value to be returned
 
        if (node.isLeaf()) {	// Just get the value of the isLeaf
